...
tables <- tibble(
  ...
  table='non_rep',
  vars_to_subset=list(
    as_tibble_col(c('child_id', 'mother_id', 'ethn1_m', 'cohort_id', 'non_existant'),
...
subset_def <- tibble(
  ...
  table=c('non_rep'),
  vars_to_subset=c(
    list(as_tibble_col(c('child_id', 'mother_id', 'ethn1_m', 'cohort_id'),
                       column_name="variable"))))

projects <- c("alspac", "gecko", "inma", "ninfea", "my_project")

missing <- tibble(folder = "2_1_core_1_0", table="non_rep", missing="non_existant")
...
test_that(".read_subset will throw error when one of headers is missing", {
  ...
  with_mock(read.csv = mock(df), {
    expect_error(.read_subset('broken.csv'), ".csv file must contain exactly three columns entitled 'folder', 'table' and 'variable'", fixed=TRUE)
  })
})
...
test_that(".read_subset will return correct output", {
  ...
  expected <- tibble(folder='outcome', table='yearlyrep', subset_vars=list(as_tibble_col(c('row_id', 'child_id', 'int_raw_3'), column_name="variable")))
...
test_that("armadillo.subset_definition should return proper subset definition", {
  df <- data.frame(
    folder=c('outcome', 'outcome', 'outcome'), 
    table=c('yearlyrep', 'yearlyrep', 'yearlyrep'),
    variable=c('row_id', 'child_id', 'int_raw_3'))
  ...
  expected <- tibble(folder='outcome', table='yearlyrep', 
                     vars_to_subset=list(as_tibble_col(c('row_id', 'child_id', 'int_raw_3'), 
                                                       column_name="variable")))
...
test_that("armadillo.subset_definition will throw error when vars are NULL", {
  ...
  with_mock(read.csv = mock(df), {
    expect_error(armadillo.subset_definition(NULL), "You must provide a .csv file with variables and tables to subset", fixed=TRUE)
  })
})
...
test_that(".get_tables throws error when tables are missing", {
  ...
  subset_def <- tibble(
    ...
    table=c('yearly_rep'), 
    vars_to_subset=c(
      list(as_tibble_col(c('row_id', 'child_id', 'int_raw_3'), 
                         column_name="variable"))))
  
  with_mock(armadillo.list_tables = mock(tables), {
    expect_error(.get_tables("test-project", subset_def), 
                 "The following folders & tables: [ outcome ] are included in your reference object, but don't exist\n  within the specified projectThe following folders & tables: [ yearly_rep ] are included in your reference object, but don't exist\n  within the specified project", 
                 fixed=TRUE)
  })
})
...
test_that(".get_tables returns tables", {
  ...
  expected <- tibble(
    ...
    table='non_rep', 
    vars_to_subset=list(
      as_tibble_col(c('child_id', 'mother_id', 'ethn1_m', 'cohort_id'), 
                    column_name="variable")),
    data=list(core))
...
test_that("armadillo.subset fails if subset_def is NULL", {
  expect_error(armadillo.subset("gecko", "my_project", NULL), 
               "You must provide an object created by armadillo.subset_definition containing details
...
test_that(".check_available_vars will return table with non existant variable", {
  expected <- tibble(folder = "2_1_core_1_0", table="non_rep", missing="non_existant")
    ...
    with_mock(armadillo.list_projects = mock(projects), {
      with_mock("MolgenisArmadillo:::.check_available_vars" = mock(missing), {
        with_mock("MolgenisArmadillo:::.make_subset" = mock(), {
          expect_identical(armadillo.subset("gecko", "my_project", subset_def), missing)
...
test_that("armadillo.subset will return missing if dryrun == TRUE and shouldn't run make_subset", {
  with_mock("MolgenisArmadillo:::.get_tables" = mock(tables), {
    with_mock(armadillo.list_projects = mock(projects), {
      with_mock("MolgenisArmadillo:::.check_available_vars" = mock(missing), {
        with_mock("MolgenisArmadillo:::.make_subset" = mock(), {
          expect_identical(armadillo.subset("gecko", "my_project", subset_def, TRUE), missing)
...
test_that(".make_subset creates new project if provided project doesn't exist and uploads table", {
  with_mock(armadillo.list_projects = mock(projects), {
...
test_that(".make_subset creates doesn't create project if provided project exists and uploads table", {
  with_mock(armadillo.list_projects = mock(projects), {
    with_mock(armadillo.create_project = mock(), {
      with_mock(armadillo.upload_table = mock(), {
