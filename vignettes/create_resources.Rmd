...
#> Error in curl::curl_fetch_memory(url, handle = handle): SSL peer certificate or SSH remote key was not OK: [armadillo3.demo.molgenis.net] SSL certificate problem: self signed certificate
```
...
#> 	/var/folders/sy/d5l5fdyj6z98mgclrl_bbkzr0000gn/T//Rtmpef3Onx/downloaded_packages
library(resourcer)
...
`yoururl + /storage/projects/{project name}/objects/{folder name}%2F{filename}` and example is:
`https://armadillo3.demo.molgenis.net/storage/projects/omics/objects/ewas%2Fgse66351_1.rda`. Here the URL is
`https://armadillo3.demo.molgenis.net`, the project name is `omics`, the fodler is `ewas` and the name of the
