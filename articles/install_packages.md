...
To be able to test your package as a developer you can install your
package using the MolgenisArmadillo. There are a few preconditions in
other to use the install_packages function.
...
execute the following endpoint. The selected profile is always `default`
...
In order to install the packages on a specific profile you can specify
the profile.
...
To be able to use new packages in the Armadillo you need to add them to
...
You are able to whilist mulitple packages as well using the following
line:
